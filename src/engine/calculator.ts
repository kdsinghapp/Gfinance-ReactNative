...
export function buildProfile(quiz?: Quiz) {
  ...
  // Horizonte (Años reales)
  const hRaw = quiz?.raw?.horiz;
  const h = (typeof hRaw === 'string') ? parseFloat(hRaw) : hRaw;
  if (h !== undefined) {
    if (h < 1 || quiz?.P_Horiz === 'Corto') {
    ...
    } else if (h > 2 && h <= 7 || quiz?.P_Horiz === 'Medio') {
    ...
    } else if (h > 7 || quiz?.P_Horiz === 'Largo') {
  ...
  const isAggressive = (h !== undefined && h > 7 || quiz?.P_Horiz === 'Largo') && (quiz?.raw?.toler === 3 || quiz?.P_Toler === 'Alta');
...
export function calculateProjectionTable(
  ...
  const yearsList = [1, 2, 3, 5, 7, 10, 15];
  const results: ProjectionRow[] = [];

  yearsList.forEach((y) => {
    const ratePessimistic = getWeightedRate(weights, 'pessimistic', y);
    const rateNeutral = getWeightedRate(weights, 'neutral', y);
    const rateOptimistic = getWeightedRate(weights, 'optimistic', y);

    results.push({
      horizon: y,
      pessimisticFV: futureValue(capital, monthly, ratePessimistic, y, frequency),
      neutralFV: futureValue(capital, monthly, rateNeutral, y, frequency),
      optimisticFV: futureValue(capital, monthly, rateOptimistic, y, frequency),
      pessimisticCAGR: ratePessimistic * 100,
      neutralCAGR: rateNeutral * 100,
      optimisticCAGR: rateOptimistic * 100,
    });
  });
